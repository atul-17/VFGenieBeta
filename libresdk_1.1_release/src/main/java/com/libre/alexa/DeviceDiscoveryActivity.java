...
import com.libre.alexa.receivers.GpsLocationReceiver;
import com.libre.alexa.receivers.WifiConnectionReceiver;
...
public class DeviceDiscoveryActivity extends UpnpListenerActivity {
    ...
    private NetworkStateReceiver networkReciever;
    ...
    public static final int REQUEST_ID_LOCATION_PERMISSION = 111;
    private static final int REQUEST_CODE_PERMISSION = 1;

    public static boolean isNetworkChangeDialogCalled = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        registerReceiver(new WifiConnectionReceiver(), new IntentFilter("android.net.wifi.WIFI_STATE_CHANGED"));
        registerReceiver(gpsLocationReceiver, new IntentFilter(LocationManager.PROVIDERS_CHANGED_ACTION));
        networkReciever = new NetworkStateReceiver();
        ...
        SSID = getconnectedSSIDname(DeviceDiscoveryActivity.this);
    ...
    Object busEventListener = new Object() {
        ...
        */
        ...
        public void newMessageRecieved(NettyData nettyData) {
            ...
            if (dummyPacket.getCommand() == 3) {
                byte[] a = nettyData.getMessage();

                short int16 = (short) (((a[0] & 0xFF) << 8) | (a[1] & 0xff));
                if (int16 != 0) {
                    LibreApplication.deviceRemoteID = int16;
                } else {
                    LibreApplication.deviceRemoteID = 0;

                }
                readLedControlValue(nettyData.remotedeviceIp);
            }
            if (libreDeviceInteractionListner != null) {
                // LSSDPNodesUpdate updateNode = getLssdpNodesUpdate(nodes.getIP());
            ...
            if (deviceDevice != null) {
                if (LibreApplication.isApplicationActiveOnUI()) {
                    if (LSSDPNodeDB.getInstance().getTheNodeBasedOnTheIpAddress(deviceDevice.getmIpAddress()) != null) {
                        //show Ls9 not found/removed when wifi is connected
                        //otherwise the wifi connected dialog is shown in WifiReciver
                        if (LibreApplication.wifiConnected) {
                            utilClass.buildCustomFullScreenForLs9DeviceNotFound(DeviceDiscoveryActivity.this, new ONButtonClickFromCustomAlertDialog() {
                                @Override
                                public void onButtonClick(@NotNull String buttonName) {
                                    LSSDPNodes nodes = LSSDPNodeDB.getInstance().getTheNodeBasedOnTheIpAddress(deviceDevice.getmIpAddress());
                                    GoAndRemoveTheDevice(nodes.getIP());
                                    //calling m-search
...
//
//        }
        checkLocationIsEnabled(DeviceDiscoveryActivity.this);
...
//        Intent mStartActivity = new Intent(context, VodafoneLoginRegisterWebViewActivity.class);
...
//        Intent mStartActivity = new Intent(context, DeviceDiscoveryActivity.class);
//        int mPendingIntentId = 123456;
//        PendingIntent mPendingIntent = PendingIntent.getActivity(context, mPendingIntentId,    mStartActivity, PendingIntent.FLAG_CANCEL_CURRENT);
//        AlarmManager mgr = (AlarmManager)context.getSystemService(Context.ALARM_SERVICE);
...
//    }
    ...
    public Boolean validateUserLocationPermission() {
...
//                Toast.makeText(getApplicationContext(), "Network is disconnected", Toast.LENGTH_SHORT).show();
            ...
            if (ssid == null || ssid.equals("") || ssid.equals("<unknown ssid>"))
                return;
            if (!LibreApplication.doneLocationChange) {
                if (checkPermissions() && isLocationEnabled()) {
                    alertForNetworkChange(DeviceDiscoveryActivity.this);
                    LibreLogger.d(this, "suma in n/w change if :SSId:" + ssid + LibreApplication.doneLocationChange);
                }
            } else {
                if (mRequestingLocationUpdates == true && !ssid.equals("") || !ssid.equals("<unknown ssid>")) {
                    //other than ble activity call network change alert box
                    if (LibreApplication.doneLocationChange) {
                        alertForNetworkChange(DeviceDiscoveryActivity.this);
                    }

                    LibreLogger.d(this, "suma in n/w change else :SSId:" + ssid
                            + LibreApplication.doneLocationChange + "mrequesting\n"
                            + mRequestingLocationUpdates + "wifiConnection: "+LibreApplication.wifiConnected);
    ...
    public boolean checkLocationIsEnabled(Context context) {
        ...
        if (locationManager != null && !LibreApplication.doneLocationChange) {
...
//        MavidApplication.doneLocationChange = true;
    ...
    private void alertForNetworkChange(Context context) {
        {
            isNetworkChangeDialogCalled = true;
            AlertDialog.Builder builder = new AlertDialog.Builder(DeviceDiscoveryActivity.this);
            String mMessage = String.format(getString(R.string.restartTitle));//, ssid, LibreApplication.mActiveSSIDBeforeWifiOff);
            builder.setMessage(mMessage)
                    .setCancelable(false)
